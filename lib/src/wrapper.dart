    ...
    return RepaintBoundary(
      child: MediaQuery(
        data: _activeKeyboard != null
            ? data.copyWith(
                viewInsets: _removeViewInsetsBottom ? null : data.viewInsets.copyWith(bottom: _bottomInset + data.viewInsets.bottom + data.padding.bottom),
              )
            : data,
        child: Stack(
          children: [
            if (widget.isBottomSheet) _buildBottomSheet() else if (widget.isDialog) _buildDialog() else _buildNormal(),
  ...
  void onKey(CustomKeyboardEvent key) {
    ...
    switch (key.type) {
      case CustomKeyType.character:
        replaceSelectionFormat(newText: key.value!);
        break;
      ...
      case CustomKeyType.clear:
        ...
        replaceSelectionFormat(selection: TextSelection(baseOffset: 0, extentOffset: origText.length));
        break;
